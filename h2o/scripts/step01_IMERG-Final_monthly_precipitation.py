...
def main(download = False):
    ...
    if download:
        ...
        earthaccess.download(results, str(CWD.parent.joinpath('data/IMERG-Final_monthly')))
    file_list = glob.glob(str(CWD.parent.joinpath('data/IMERG-Final_monthly/*.HDF5')))
    file_list.sort()
